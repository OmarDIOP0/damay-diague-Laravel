...
var modal = require('micromodal'); // commonjs module

var panelCreateCourse = $("#panel-create-course");
var panelCourses = $("#panel-courses");
var panelComments = $("#panel-comments");
var panelDetailCours = $("#panel-detail-cours");

var panels = [panelCourses, panelCreateCourse, panelComments, panelDetailCours];
...
$("#btn-add-lesson").click(() => {
    modal.show('modal-add-lesson');
});
...
function showDetailCours(courseId, courseName) {
    ...
    getChapter(courseId, displayChapters);
...
function displayCourses(coursesList) {
    ...
    // <td>${c.name} </td>
    // <td>${c.created_at}</td>
    // <td><img  class="search-icon" src="/images/add-course-icon.png" name="${c.name}" data-usage=${c.id}></td>
    // </tr>`;
...
$("#btn-create-chapter").click(() => {
    modal.show('modal-1');
})

$("#form-add-chapter").submit((e) => {
    e.preventDefault();
    var courseName = $("#course-name").text();
    var chapterName = $("input[name='chapter-name']").val();
    var cours = courses.filter(elt => elt.name == courseName)[0];
    var coursId = $("input[name='course-id']").val();

    $.post(`/cours/${cours.slug}/chapter/create`, {
...
function getChapter(courseId, callBack) {
    $.getJSON(`/cours/${courseId}/chapter/all`).done(response => {
        callBack(response)
        console.log(response);
    }).fail(error => {
        console.log(error);
...
$("#chapter-list").on('click', '.add-lesson', (e) => {
    modal.show('modal-add-lesson');

    let chapterId = e.currentTarget.attributes['id'].value;
...
function displayChapters(lessons) {
    var div = $("#lesson-list");
    div.empty();
...
var tr = `
  <div class="item">
    ...
    <div class="content">
      <div class="ui header grey">${lesson.name}</div>
      An excellent companion
    </div>
  </div>`;
        var d = `<div class="lesson-item">
    <div class="lesson-item-header">
    <h2>Chapitre ${item+1} :${lesson.name}</h2>
    <div>
    <img class="add-lesson" id="${lesson.id}" data="${lesson.name}" src="/images/add-course-icon.png" width = '25px' align="middle"/>
    </div>
    </div>
    </div>`;
...
$("#btn-new-party").click(() => {
    let i = $("#parties label").length;

    var t = `<li>
            <input type="text" name="parties[]" placeholder="Nom de la partie">
            <input type="number" name="pages[]" min="0" placeholder="Numero Page">
          </li>`;
